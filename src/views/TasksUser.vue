<template>
  <div>
    ...
    <md-table v-else v-model="tasks" md-card @md-selected="onSelect">
      <md-table-toolbar>
      ...
      <md-table-row id="rows" slot="md-table-row" slot-scope="{ item }" md-selectable="multiple">
        <md-table-cell id="rows" md-label="Ocorrência" md-sort-by="ocorrencia" class="left">{{ item.task }}</md-table-cell>
        <md-table-cell id="rows" md-label="Solicitante" md-sort-by="solicitante" class="left">{{ item.requester }}</md-table-cell>
        <md-table-cell id="rows" md-label="Tipo" md-sort-by="Tipo" class="left">{{ item.type }}</md-table-cell>
        <md-table-cell id="rows" md-label="Prazo" md-sort-by="prazo" class="left">{{ item.term }}</md-table-cell>
  ...
  export default {
    ...
    methods: {
        ...
        UpdateTasksCompleted(item){
            ...
            this.loading = true;
            axios.put(`http://localhost:3333/tasks/${item.id}`, params);
            this.UpdateTasks()
        },
        ...
        UpdateAllTasksPending (){
            ...
            axios.put(`http://localhost:3333/tasks/all/pending`, this.selected).then(()=>{
                setTimeout(() => {
                    this.UpdateTasks()
                }, 2000);
            })
            ...
            axios.put(`http://localhost:3333/tasks/all/progress`, this.selected).then(()=>{
                setTimeout(() => {
                    this.UpdateTasks()
                }, 2000);
            })
        ...
        UpdateAllTasksCompleted (){
            ...
            axios.put(`http://localhost:3333/tasks/all/completed`, this.selected).then(()=>{
                setTimeout(() => {
                    this.UpdateTasks()
                }, 2000);
            })
        ...
        DeleteTaks(item){
            this.loading = true;
            axios.delete(`http://localhost:3333/tasks/delete/${item.id}`);
            this.UpdateTasks()
        },
        DeleteAllTasks(){
            this.loading = true;
            axios.post(`http://localhost:3333/tasks/delete/all`, this.selected).then(() =>{
                setTimeout(() => {
                    this.UpdateTasks()
                }, 2000);
            })
        ...
        UpdateTasks(){
           setTimeout(() => {
                axios.get('http://localhost:3333/tasks/1').then(res=>{
                    this.loading = false;
                    this.tasks = res.data;
                })
           }, 1000);
